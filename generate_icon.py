#!/usr/bin/env python3
"""Generate Player Portals' mod menu icon: two frames, tied to each other.

Two portals and the line between them is the whole mod, and it is the one thing a
nether portal cannot show you: which other portal it goes to. Colours are the
vanilla obsidian and nether portal textures' own.
...
OUT = os.path.join(HERE, "src/main/resources/assets/player-portals-justfatlard/icon.png")
...
# Two frames facing each other across the icon, and the tie between them.
FRAME_TOP, FRAME_BOTTOM = 4, 12
LEFT_FRAME = (2, 5)
RIGHT_FRAME = (10, 13)
TIE_Y = 8
TIE_X = (7, 8)
...
def build_icon():
    ...
    glow = portal_light()
    icon = [[dim(px, 1.35) for px in row] for row in vanilla("block/obsidian.png")[:16]]

    for x0, x1 in (LEFT_FRAME, RIGHT_FRAME):
        for y in range(FRAME_TOP - 1, FRAME_BOTTOM + 2):
            for x in range(x0 - 1, x1 + 2):
                # An obsidian lip around the sheet, so a portal reads as a portal in a frame
                if FRAME_TOP <= y <= FRAME_BOTTOM and x0 <= x <= x1:
                    icon[y][x] = glow if (x + y) % 3 else dim(glow, 0.7)
                else:
                    icon[y][x] = outline if (x + y) % 2 else dark

    # The tie: two sparks, not a solid line. A line would read as a bridge you walk.
    for x in TIE_X:
        icon[TIE_Y][x] = glow
    return scale(icon, 8)
